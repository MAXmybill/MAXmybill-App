...
class SaleSyncService {
  ...
  Future<void> _setFirestoreNetworkEnabled(bool enabled) async {
    try {
      if (enabled) {
      ...
      print('⚠️ Failed to toggle Firestore network: $e');
    }
  }
  ...
  Future<void> init() async {
    ...
    _lastOnline = !initialConnectivity.contains(ConnectivityResult.none);
    await _setFirestoreNetworkEnabled(_lastOnline);
    ...
    _connectivitySub = Connectivity().onConnectivityChanged.listen((result) {
      ...
      final cameOnline = !_lastOnline && isOnline;
      final wentOffline = _lastOnline && !isOnline;
      _lastOnline = isOnline;

      if (cameOnline) {
        _setFirestoreNetworkEnabled(true);
        print('🌐 Connection detected! Starting sync...');
        syncAll();
      } else if (wentOffline) {
        _setFirestoreNetworkEnabled(false);
        print('📵 No connection detected');
      } else {
        print(isOnline ? '📶 Still online, skip duplicate sync trigger' : '📵 No connection detected');
      }
    });
  ...
  Future<bool> saveSale(Sale sale) async {
    if (!_initialized || _box == null || !_box!.isOpen) {
      await init();
    }

    // Always persist locally first so billing can complete even when backend is unavailable.
    ...
    if (await _isOnline()) {
      try {
        ...
        if (_isTemporaryNetworkError(e)) {
          // Keep offline copy and let background sync retry when connectivity recovers.
          _lastOnline = false;
          _nextRetryAt = DateTime.now().add(_retryCooldown);
          await _setFirestoreNetworkEnabled(false);
          print('⚠️ Immediate sync failed for ${sale.id}, queued for retry: $e');
          return false;
        }
        rethrow;
      }
  ...
  Future<void> syncAll() async {
    ...
    if (_nextRetryAt != null && now.isBefore(_nextRetryAt!)) {
      print('⏸️ Sync cooldown active until $_nextRetryAt');
      return;
    }

    if (!await _isOnline()) {
      print('📵 Offline detected, skipping syncAll');
      return;
    }
      ...
      for (var sale in unsyncedSales) {
        ...
        try {
          ...
          if (_isTemporaryNetworkError(e)) {
            ...
            _nextRetryAt = DateTime.now().add(_retryCooldown);
            await _setFirestoreNetworkEnabled(false);
            print('⏸️ Temporary network issue detected, pausing further sync attempts until $_nextRetryAt');
            break;
          }
        }
  ...
  bool _isTemporaryNetworkError(Object error) {
    ...
    return text.contains('unknownhostexception') ||
        ...
        text.contains('keepalive failed') ||
