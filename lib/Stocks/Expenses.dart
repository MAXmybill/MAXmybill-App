...
class _ExpensesPageState extends State<ExpensesPage> {
                  ...
                  return StreamBuilder<QuerySnapshot>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final expenses = snapshot.data!.docs.where((doc) {
                        ...
                        final expenseDate = ts.toDate();
                        if (_selectedDate != null &&
                            !_isSameDay(expenseDate, _selectedDate!))
                          return false;

                        if (_searchQuery.isEmpty) return true;
                        final name = (data['expenseName'] ?? '')
                            .toString()
                            .toLowerCase();
  ...
  Widget _buildExpenseCard(
    ...
    final dateStr = ts != null
        ...
        : 'N/A';
    final expenseNumber =
        (data['expenseNumber'] ?? data['referenceNumber'] ?? '').toString();
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text(
                          expenseNumber.isNotEmpty ? expenseNumber : 'Expense',
                          style: const TextStyle(
                            fontWeight: FontWeight.w900,
                            color: kPrimaryColor,
                            fontSize: 13,
...
class _CreateExpensePageState extends State<CreateExpensePage> {
  ...
  final TextEditingController _paidAmountController =
      TextEditingController(); // replaces _creditAmountController
  ...
  Future<void> _loadExpenseNameSuggestions() async {
    try {
      ...
      if (names.isEmpty) {
        try {
          final expCol = await FirestoreService().getStoreCollection(
            'expenses',
          );
  ...
  Future<void> _saveExpense() async {
    ...
    try {
      ...
      await FirestoreService().addDocument('expenses', {
        ...
        'timestamp': Timestamp.fromDate(expenseDateTime),
        'uid': widget.uid,
  ...
  Future<void> _saveExpenseName(String name) async {
    try {
      ...
      if (query.docs.isNotEmpty) {
        await col.doc(query.docs.first.id).update({
  ...
  Widget _buildAutocompleteExpenseName() {
    return Autocomplete<String>(
      ...
      fieldViewBuilder: (ctx, ctrl, focus, onSub) {
        if (_expenseNameController.text.isNotEmpty && ctrl.text.isEmpty)
          ctrl.text = _expenseNameController.text;
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          isExpanded: true,
          icon: const HeroIcon(HeroIcons.chevronDown, color: kBlack54),
          style: const TextStyle(
            fontWeight: FontWeight.w600,
            fontSize: 14,
            color: kBlack87,
          ...
          onChanged: (v) async {
            if (v == 'Add Expense Type') {
              final res = await showDialog<String>(
                context: context,
                builder: (_) => AddExpenseTypePopup(uid: widget.uid),
              );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const HeroIcon(HeroIcons.trash, color: kWhite),
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          expenseData['expenseName'] ?? 'General Expense',
                          style: const TextStyle(
                            color: kOrange,
                            fontSize: 15,
                            fontWeight: FontWeight.w700,
                    ...
                    if (expenseData['taxAmount'] != null &&
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Text(
                            '$currencySymbol${expenseData['taxAmount']}',
                            style: const TextStyle(
                              fontSize: 11,
                              fontWeight: FontWeight.w700,
                              color: kBlack87,
