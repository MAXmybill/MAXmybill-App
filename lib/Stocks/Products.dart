...
class ProductsPage extends StatefulWidget {
  ...
  const ProductsPage({super.key, required this.uid, this.userEmail});
...
class _ProductsPageState extends State<ProductsPage> {
      ...
      floatingActionButton: (isAdmin || _hasPermission('addProduct'))
          ? FloatingActionButton.extended(
              onPressed: () => Navigator.push(
                ...
                CupertinoPageRoute(
                  builder: (c) =>
                      AddProductPage(uid: _uid, userEmail: widget.userEmail),
                ),
              ),
  ...
  Widget _buildProductCard(
    ...
    final stock = _parseStockValue(data['currentStock']);
    final lowStockLimit = _resolveLowStockLimit(data['lowStockAlert']);
    ...
    final isLowStock = stockEnabled && stock > 0 && stock <= lowStockLimit;
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          onTap: _isMultiSelectMode
              ...
              : null,
          onLongPress:
              (isAdmin || _hasPermission('addProduct')) && !_isMultiSelectMode
              ? () => setState(() {
                            ...
                            Flexible(
                              child: Text(
                                taxesList
                                    .map(
                                      (t) =>
                                          '${t['name']} (${(t['percentage'] as num).toDouble().toStringAsFixed(1)}%)',
                                    )
                                    .join(' + '),
                                style: const TextStyle(
                                  fontSize: 10,
  ...
  Widget _buildSortOption(String label, String value, HeroIcons icon) {
    ...
    return ListTile(
      ...
      trailing: isSelected
          ? HeroIcon(
              _sortAscending ? HeroIcons.arrowUp : HeroIcons.arrowDown,
              color: kPrimaryColor,
              size: 16,
            )
          : null,
  ...
  Widget _buildFilterOption(
    ...
    return ListTile(
      ...
      title: Text(
        ...
        style: TextStyle(
          fontWeight: isSelected ? FontWeight.bold : FontWeight.w600,
          color: isSelected ? color : kBlack87,
          fontSize: 14,
        ),
      ),
      trailing: isSelected
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  _qtyBtn(HeroIcons.minus, () {
                    ...
                    if (v > 0)
                      setDialogState(
                        () => ctrl.text = AmountFormatter.format(v - 1),
                      );
                  }),
                  Expanded(
                    child: ValueListenableBuilder<TextEditingValue>(
              ...
              Text(
                'Current in record: ${AmountFormatter.format(current)}',
                style: const TextStyle(
                  color: kBlack54,
                  fontSize: 11,
                  fontWeight: FontWeight.w600,
  ...
  Widget _qtyBtn(HeroIcons i, VoidCallback onTap) => InkWell(
    onTap: onTap,
    borderRadius: BorderRadius.circular(10),
    child: Container(
      ...
      child: HeroIcon(i, color: kPrimaryColor, size: 24),
    ),
  );
  ...
  void _showDeleteConfirmDialog(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to delete "${data['itemName']}"? This action is permanent.',
  ...
  void _showBulkDeleteConfirmDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to delete ${_selectedProductIds.length} products? This action is permanent and cannot be undone.',
          style: const TextStyle(color: kBlack54, fontSize: 14),
        ),
        actions: [
          TextButton(
              ...
              try {
                ...
                if (mounted) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        '${_selectedProductIds.length} products deleted successfully',
                      ),
  ...
  List<QueryDocumentSnapshot> _filterAndSortProducts(
    ...
    var list = items.where((doc) {
      ...
      final stockEnabled = data['stockEnabled'] == true;
      if (!name.contains(_searchQuery) && !barcode.contains(_searchQuery))
        return false;
      ...
      if (_filterStock == 'lowStock')
        return stock > 0 && stock <= lowStockLimit;
      if (_filterStock == 'inStock') return stock > 0;
      return true;
    }).toList();
    ...
    list.sort((a, b) {
      ...
      if (_sortBy == 'name') {
        ...
        if (numA != null && numB != null)
          res = numA.compareTo(numB);
        else if (numA != null)
          res = -1;
        else if (numB != null)
          res = 1;
        else
          res = (rawA?.toString() ?? '').compareTo(rawB?.toString() ?? '');
      } else if (_sortBy == 'price') {
        res = (dA['price'] ?? 0).compareTo(dB['price'] ?? 0);
      } else {
        final stockA = _parseStockValue(dA['currentStock']);
        final stockB = _parseStockValue(dB['currentStock']);
        res = stockA.compareTo(stockB);
      }
  ...
  double _parseStockValue(dynamic raw) {
    if (raw is num) return raw.toDouble();
    return double.tryParse(raw?.toString() ?? '0') ?? 0;
  }

  double _resolveLowStockLimit(dynamic rawAlert) {
    final parsed = _parseStockValue(rawAlert);
    // Match report behavior: if alert is missing/invalid, use 5 as default low-stock threshold.
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              textAlign: TextAlign.center,
              style: TextStyle(fontSize: 15, color: kBlack54, height: 1.5),
            ),
          ],
        ),
      ),
