...
import 'package:maxmybill/Stocks/AddCategoryPopup.dart';
...
class CategoryPage extends StatefulWidget {
  ...
  const CategoryPage({super.key, required this.uid, this.userEmail});
...
class _CategoryPageState extends State<CategoryPage> {
  ...
  Future<void> _initStreams() async {
    try {
      final catStream = await FirestoreService().getCollectionStream(
        'categories',
      );
  ...
  void _showAddCategoryDialog(BuildContext context) {
    ...
    showDialog(
      context: context,
      builder: (c) => AddCategoryPopup(uid: _uid, userEmail: _userEmail),
    );
  }
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              if (ctrl.text.trim().isNotEmpty) {
                await FirestoreService().updateDocument('categories', id, {
                  'name': ctrl.text.trim(),
                });
                Navigator.pop(c);
              }
    ...
    showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              '${context.tr('are_you_sure_delete')} "$name"?',
  ...
  Future<void> _deleteCategoryAndUpdateProducts(
    String categoryId,
    String categoryName,
  ) async {
    try {
      ...
      if (_productsRef != null) {
        final productsInCategory = await _productsRef!
...
class _CategoryDetailsPageState extends State<CategoryDetailsPage> {
  ...
  @override
  Widget build(BuildContext context) {
    final canAddCategory = PermissionHelper.getUserPermissions(widget.uid).then(
      (userData) {
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: (stock > 0 ? kGoogleGreen : kErrorColor).withOpacity(0.1),
              borderRadius: BorderRadius.circular(20),
            ),
            child: Text(
              "${stock.toInt()} IN STOCK",
              style: TextStyle(
                color: stock > 0 ? kGoogleGreen : kErrorColor,
                fontWeight: FontWeight.w900,
                fontSize: 9,
                letterSpacing: 0.5,
              ),
  ...
  void _confirmRemoveFromCategory(String productId, String productName) {
    showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        content: Text(
          'Remove "$productName" from ${widget.categoryName}?\n\nThe product will not be deleted.',
          style: const TextStyle(color: kBlack54, fontSize: 14),
        ),
        actions: [
          TextButton(
  ...
  void _showAddExistingProductDialog(BuildContext context) {
    showDialog(
      context: context,
      builder: (c) =>
          _AddExistingProductDialog(categoryName: widget.categoryName),
    );
  }
...
class _AddExistingProductDialog extends StatefulWidget {
  ...
  @override
  State<_AddExistingProductDialog> createState() =>
      _AddExistingProductDialogState();
}
                  ...
                  : StreamBuilder<QuerySnapshot>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        final products = snapshot.data!.docs.where((doc) {
                          ...
                          if ((d['category'] ?? '') == widget.categoryName)
                            return false;
