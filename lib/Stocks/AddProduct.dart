...
class _AddProductPageState extends State<AddProductPage> {
  ...
  late Future<bool> _canUseBulkInventoryFuture;
      ...
      for (var doc in existingProduct.docs) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Product Code is already mapped with $productName'),
                        ...
                        builder: (BuildContext successDialogContext) {
                          return AlertDialog(
                            ...
                            content: Column(
                              ...
                              children: [
                                Text(
                                  StorageSaver.locationMessageForPath(
                                    result,
                                    fallbackMessage:
                                        'Saved in app storage under MAXmybill/Templates',
                                  ),
  ...
  Widget _buildExpiryDateField() {
    return GestureDetector(
      onTap: () async {
        ...
        final DateTime lastDate = today.add(const Duration(days: 3650));
        DateTime initialDate = _selectedExpiryDate != null
            ? DateUtils.dateOnly(_selectedExpiryDate!)
            : today;
  ...
  Future<void> _saveProduct() async {
    ...
    if (widget.productId != null) {
      ...
      await LocalStockService().cacheStock(
        widget.productId!,
        pData['currentStock'] as double,
      );
    } else {
