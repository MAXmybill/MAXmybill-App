...
class _VendorsPageState extends State<VendorsPage> {
                    ...
                    child: _buildStat(
                      ...
                      _formatMoney(
                        _vendors.fold(
                          0.0,
                          (sum, v) =>
                              sum + ((v['totalPurchases'] ?? 0).toDouble()),
                        ),
                      ),
                      HeroIcons.banknotes,
                    ),
                    ...
                    child: _buildStat(
                      ...
                      _formatMoney(
                        _vendors.fold(
                          0.0,
                          (sum, v) =>
                              sum + ((v['pendingCredit'] ?? 0).toDouble()),
                        ),
                      ),
                      HeroIcons.wallet,
                    ),
                ...
                Row(
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        Text(
                          '$_currencySymbol${totalPurchases.toStringAsFixed(2)}',
                          style: const TextStyle(
                            fontWeight: FontWeight.w900,
                            fontSize: 15,
                            color: kGoogleGreen,
                        ...
                        if (pendingCredit > 0)
                          Text(
                            'Credit $_currencySymbol${pendingCredit.toStringAsFixed(2)}',
                            style: const TextStyle(
                              fontWeight: FontWeight.w800,
                              fontSize: 11,
                              color: kErrorColor,
  ...
  void _showAddVendorDialog(BuildContext context) {
    final nameCtrl = TextEditingController(),
        ...
        gstinCtrl = TextEditingController(),
  ...
  void _showEditVendorDialog(
    ...
    final nameCtrl = TextEditingController(text: vendor['name']),
        phoneCtrl = TextEditingController(text: vendor['phone']),
        gstinCtrl = TextEditingController(text: vendor['gstin']),
        addressCtrl = TextEditingController(text: vendor['address']);
  ...
  Future<void> _showDeleteConfirmation(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to remove "${vendor['name']}"? This action cannot be undone.',
  ...
  Widget _buildNoResults() => Center(
    child: Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        const HeroIcon(HeroIcons.magnifyingGlass, size: 64, color: kGrey300),
        const SizedBox(height: 16),
        Text(
...
// VENDOR DETAILS PAGE
// ==========================================
Future<List<Map<String, dynamic>>> _fetchSupplierPurchases(
  Map<String, dynamic> vendor,
) async {
  ...
  if (byId.isEmpty && supplierPhone.isNotEmpty) {
    try {
      final snap = await col
          .where('supplierPhone', isEqualTo: supplierPhone)
          .get();
      addDocs(snap.docs);
    } catch (_) {}
  }
...
double _remainingCreditFromNote(Map<String, dynamic> note) {
  final explicitRemaining = _toDouble(
    note['remainingAmount'] ?? note['balanceAmount'],
  );
  if (explicitRemaining > 0) return explicitRemaining;

  final amount = _toDouble(
    note['amount'] ?? note['totalAmount'] ?? note['creditAmount'],
  );
...
Future<List<Map<String, dynamic>>> _fetchSupplierCreditNotes(
  Map<String, dynamic> vendor,
) async {
  final col = await FirestoreService().getStoreCollection(
    'purchaseCreditNotes',
  );
...
Future<double> _getPendingCreditTotalForVendor(
  Map<String, dynamic> vendor,
) async {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      _buildStat(
                        _purchases.length == 1 ? 'Bill' : 'Bills',
                        _isLoading
                      ...
                      _buildStat(
                        'Total Spent',
                        '${widget.currencySymbol}${totalPurchases.toStringAsFixed(0)}',
                        kGoogleGreen,
                      ),
                      _buildStat(
                        'Credit',
                        '${widget.currencySymbol}${totalCredit.toStringAsFixed(0)}',
                        kErrorColor,
                      ),
                    ],
                  ),
...
class SupplierPurchaseHistoryPage extends StatefulWidget {
  ...
  @override
  State<SupplierPurchaseHistoryPage> createState() =>
      _SupplierPurchaseHistoryPageState();
}

class _SupplierPurchaseHistoryPageState
    extends State<SupplierPurchaseHistoryPage> {
  List<Map<String, dynamic>> _purchases = [];
  bool _isLoading = true;
...
class SupplierCreditHistoryPage extends StatefulWidget {
  ...
  const SupplierCreditHistoryPage({
    super.key,
    required this.vendor,
    required this.currencySymbol,
    required this.uid,
  ...
  @override
  State<SupplierCreditHistoryPage> createState() =>
      _SupplierCreditHistoryPageState();
}
                        ...
                        child: Column(
                          children: [
                            ...
                            Row(
                              ...
                              children: [
                                Text(
                                  'Credit: ${widget.currencySymbol}${credit.toStringAsFixed(2)}',
                                  style: const TextStyle(
                                    fontWeight: FontWeight.w900,
                                    color: kErrorColor,
                                  ),
