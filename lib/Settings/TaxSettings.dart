...
class _TaxSettingsPageState extends State<TaxSettingsPage>
  ...
  Future<void> _addNewTax() async {
    ...
    try {
      final existingTaxes = await FirestoreService().getStoreCollection(
        'taxes',
      );
      ...
      if (querySnapshot.docs.isNotEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('This tax rate already exists'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Tax category added successfully'),
            backgroundColor: kGoogleGreen,
            behavior: SnackBarBehavior.floating,
          ),
        );
              ...
              builder: (context, value, _) {
                ...
                return TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'e.g. Customs Duty',
                    prefixIcon: Icon(Icons.label_important_rounded, size: 20),
                    filled: true,
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      onPressed: () => Navigator.pop(context),
                      icon: const Icon(Icons.close_rounded, color: kBlack54),
                      style: IconButton.styleFrom(backgroundColor: kGreyBg),
                    ),
                  ],
                ),
              ),
                    ...
                    return StreamBuilder<QuerySnapshot>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        final products = snapshot.data!.docs.where((doc) {
                          final data = doc.data() as Map<String, dynamic>;
                          return (taxId != null && data['taxId'] == taxId) ||
                              (data['taxPercentage'] == taxData['percentage'] &&
            ...
            children: [
              ...
              _wrapDropdown(
                ...
                DropdownButton<String>(
                  value: _taxNames.contains(_selectedTaxName)
                      ? _selectedTaxName
                      : _taxNames.first,
                  isExpanded: true,
                  isDense: true,
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            if (!snapshot.hasData || snapshot.data!.docs.isEmpty)
              return _buildEmptyState("No tax rates configured.");
            ...
            return Container(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    onTap: () => _showTaxProducts(context, {
                      ...taxData,
                      'id': taxes[index].id,
                    }),
                    contentPadding: const EdgeInsets.symmetric(
                    ...
                    trailing: Row(
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () => _deleteTax(taxes[index].id, name),
                        ),
                        const Icon(
  ...
  void _deleteTax(String id, String name) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Remove "$name"? This will affect products mapped to this category.',
        ),
        actions: [
          TextButton(
    ...
    return FutureBuilder<Stream<QuerySnapshot>>(
      ...
      builder: (context, streamFutureSnapshot) {
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            if (!snapshot.hasData || snapshot.data!.docs.isEmpty)
              return _buildEmptyState("Configure tax rates in the first tab.");
            final taxes = snapshot.data!.docs;
            return Container(
              decoration: BoxDecoration(
                color: kWhite,
  ...
  Widget _buildEmptyState(String msg) => Center(
    child: Column(
      ...
      children: [
        ...
        Icon(Icons.receipt_long_outlined, size: 48, color: kGrey300),
        const SizedBox(height: 16),
        Text(
          msg,
          style: const TextStyle(
