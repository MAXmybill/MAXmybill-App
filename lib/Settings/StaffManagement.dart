...
import 'package:maxmybill/Menu/InviteStaffPage.dart';
import 'package:maxmybill/Settings/RoleManagement.dart';
import 'package:maxmybill/Settings/PermissionEditor.dart';
...
class _StaffManagementPageState extends State<StaffManagementPage> {
  ...
  late Future<int> _maxStaffCountFuture;
  bool _accessLoaded = false;
  bool _canAccessStaffManagement = false;
  bool _isOwner = false;

  @override
  void initState() {
    super.initState();
    _maxStaffCountFuture = PlanPermissionHelper.getMaxStaffCount();
  ...
  Future<void> _loadAccess() async {
    ...
    setState(() {
      _isOwner = role == 'owner';
      _canAccessStaffManagement =
          _isOwner || permissions['staffManagement'] == true;
      _accessLoaded = true;
    });
  }
    ...
    try {
      ...
      if (verifiedCount > 0 && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              "✅ Updated status for $verifiedCount staff member(s)",
            ),
        ...
        builder: (context) {
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                const Text(
                  "Enter the staff's temporary password to check their status.",
    ...
    try {
      ...
      if (cred.user?.emailVerified ?? false) {
        ...
        if (mounted)
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('✅ Verified! You can now approve.'),
              backgroundColor: kGoogleGreen,
            ),
          );
      } else {
  ...
  Stream<QuerySnapshot> _getStaffStream() {
    // Read full staff collection so legacy users (without invitedBy) are still visible.
    return _firestoreService
        .getStoreCollection('users')
        .then(
          (collection) => collection.snapshots(includeMetadataChanges: true),
        )
        .asStream()
  ...
  Widget build(BuildContext context) {
    ...
    if (!_canAccessStaffManagement) {
      return Scaffold(
        ...
        body: const Center(
          child: Text(
            'You do not have access to Staff Management.',
            style: TextStyle(
              fontSize: 14,
    ...
    return PopScope(
      ...
      child: Scaffold(
          ...
          actions: [
              ...
              IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    CupertinoPageRoute(
                      builder: (_) => RoleManagementPage(uid: widget.uid),
                    ),
                  );
                },
              ),
            IconButton(
              icon: _isCheckingVerifications
                  ...
                  : const Icon(Icons.refresh_rounded, color: kWhite, size: 22),
              onPressed: _isCheckingVerifications
                  ? null
                  : _checkAllPendingVerifications,
            ),
          ],
        ...
        floatingActionButton: _isOwner
            ? FutureBuilder<int>(
                future: _maxStaffCountFuture,
                builder: (context, snapshot) {
                  final isLocked = (snapshot.data ?? 0) <= 0;
                  return FloatingActionButton.extended(
                    ...
                    label: Text(
                      isLocked ? 'Invite Staff (Premium)' : 'Invite Staff',
                      style: TextStyle(
                    ...
                    if (!isActive)
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          child: Text(
                            isEmailVerified ? "Approve" : "Check Status",
                            style: const TextStyle(
                              fontSize: 10,
                              fontWeight: FontWeight.w800,
                              color: kWhite,
    ...
    return PopupMenuButton<String>(
      ...
      onSelected: (v) async {
        if (v == 'toggle') {
          ...
          final currentPerms = Map<String, bool>.from(
            permissions.map((k, v) => MapEntry(k, v == true)),
          );
          ...
          if (result != null) {
            ...
            await FirebaseFirestore.instance
                .collection('users')
                .doc(staffId)
                .set({'permissions': result}, SetOptions(merge: true))
                .catchError((_) {});
          }
        } else if (v == 'edit')
      ...
      itemBuilder: (_) => [
        ...
        _menuItem(
          ...
          isActive ? kErrorColor : kGoogleGreen,
        ),
        _menuItem(
          'delete',
  ...
  void _showStaffDetailsDialog(
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text(
          n,
          style: const TextStyle(fontWeight: FontWeight.w800, color: kBlack87),
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            _info("Email", e),
            _info("Phone", p),
            _info("Status", a ? "Active" : "Inactive"),
          ],
        ),
        actions: [
          TextButton(
  ...
  void _showDeleteConfirmation(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text(
          'Remove Staff Member?',
          style: TextStyle(fontWeight: FontWeight.w800, color: kBlack87),
        ),
        content: Text(
          'This will permanently delete access for $name. They won\'t be able to login again with their email/password.',
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                final staffDoc = await _firestoreService
                    .getStoreCollection('users')
  ...
  Future<void> _deleteStaffAuthAccount(
    String email,
    String? tempPassword,
  ) async {
    FirebaseApp? tempApp;
      ...
      if (tempPassword != null && tempPassword.isNotEmpty) {
        try {
          ...
          if (e.code == 'user-not-found') {
            debugPrint(
              '⚠️ Auth account not found for: $email (may already be deleted)',
            );
          } else if (e.code == 'wrong-password') {
            debugPrint(
              '⚠️ Cannot delete auth account - password mismatch for: $email',
            );
          } else {
            debugPrint(
              '⚠️ Error deleting auth account for $email: ${e.message}',
            );
          }
        }
      } else {
        debugPrint(
          '⚠️ No tempPassword available for $email - cannot delete auth account',
        );
      }
    } catch (e) {
  ...
  void _showEditStaffDialog(
    ...
    String cr,
    bool ca,
    Map<String, dynamic> cprms,
  ) {
    final nameC = TextEditingController(text: cn);
    String role = cr;
    setState(() => _isDialogOpen = true);
    showDialog(
      context: ctx,
    ...
    child: Column(
      ...
      children: [
        ...
        Text(
          ...
          style: const TextStyle(color: kBlack54, fontFamily: 'Lato'),
        ),
      ],
    ),
  );
        ...
        return Container(
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              value: current,
              isExpanded: true,
              icon: const Icon(Icons.arrow_drop_down_rounded, color: kBlack54),
              style: const TextStyle(
                fontWeight: FontWeight.w600,
                fontSize: 14,
                color: kBlack87,
...
class _StaffPermissionsPageState extends State<StaffPermissionsPage> {
  ...
  void initState() {
    ...
    perms = {
      ...
      'staffSalesReport':
          widget.currentPermissions['staffSalesReport'] ?? false,
