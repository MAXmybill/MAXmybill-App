...
import 'package:maxmybill/Menu/Menu.dart'
    hide
        kWhite,
        kPrimaryColor,
        kErrorColor,
...
class _CustomerDetailsPageState extends State<CustomerDetailsPage> {
  ...
  Future<void> _confirmDelete(BuildContext context) async {
    final dueAmount = await _getOutstandingDue();
    if (dueAmount > 0.009) {
      if (!mounted) return;
      await _showDeleteBlockedDialog(context, dueAmount);
      return;
    }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "This action cannot be undone. All customer data and credit history will be removed.",
                textAlign: TextAlign.center,
                style: TextStyle(color: kBlack54, fontSize: 14, height: 1.4),
              ),
              const SizedBox(height: 24),
              SizedBox(
  ...
  Future<double> _getOutstandingDue() async {
    final latestDue = await LedgerHelper.computeClosingBalance(
      widget.customerId,
      syncToFirestore: true,
    );
    final savedDue = _parseAmount(widget.customerData['balance']);
    return latestDue > savedDue ? latestDue : savedDue;
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text(
                'Customer has pending credit due of $_currencySymbol${dueAmount.toStringAsFixed(2)}. Clear due before deleting.',
                textAlign: TextAlign.center,
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    await FirestoreService()
                        .updateDocument('customers', widget.customerId, {
              ...
              Row(
                ...
                children: [
                  _buildPaymentToggle(
                    ...
                    HeroIcons.banknotes,
                    selectedMethod,
                    (v) => setModalState(() => selectedMethod = v),
                  ),
                  _buildPaymentToggle(
                    "Online",
                    HeroIcons.qrCode,
        ...
        children: [
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: isActive ? kPrimaryColor : kGrey200,
                width: 1.5,
              ),
            ),
            child: Center(
          ...
          Text(
            ...
            style: TextStyle(
              ...
              color: isActive ? kPrimaryColor : kBlack54,
            ),
          ),
        ],
      ),
        ...
        builder: (context, setDialogState) {
          return AlertDialog(
            ...
            actions: [
              ...
              if (customerData['rating'] != null &&
                  (customerData['rating'] as num) > 0)
                TextButton(
                  onPressed: () {
  ...
  Future<void> _processTransaction(
    ...
    try {
      ...
      final paymentReceiptPrefix =
          await NumberGeneratorService.getPaymentReceiptPrefix();
      final paymentReceiptNumber =
          await NumberGeneratorService.generatePaymentReceiptNumber();
      ...
      final paymentReceipts = await FirestoreService().getStoreCollection(
        'paymentReceipts',
      );
        ...
        Navigator.push(
          ...
          CupertinoPageRoute(
            builder: (_) => PaymentReceiptPage(
              ...
              currency: CurrencyService.getSymbolWithSpace(
                storeData?['currency'],
              ),
              isManualCredit: true,
            ),
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<DocumentReference>(
      future: FirestoreService().getDocumentReference(
        'customers',
        widget.customerId,
      ),
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () =>
                                  _navigateToEditCustomer(context, data),
                            ),
                          ],
                        ),
                        const SizedBox(height: 16),
                        ...
                        _buildInfoRow(
                          ...
                          "GST No",
                          data['gst'] ?? data['gstin'] ?? 'Not Provided',
                        ),
                        const SizedBox(height: 10),
                        _buildInfoRow(
                          HeroIcons.mapPin,
                        ...
                        _buildInfoRow(
                          ...
                          "Default Discount",
                          "${(data['defaultDiscount'] ?? 0).toString()}%",
                        ),
                        const SizedBox(height: 10),
                        _buildInfoRow(
                          HeroIcons.cake,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "$_currencySymbol${amt.toStringAsFixed(2)}",
              style: TextStyle(
                color: color,
                fontSize: 15,
                fontWeight: FontWeight.w900,
            ...
            () {
              if (title == "Bill History") {
                Navigator.push(
                  context,
                  CupertinoPageRoute(
                    builder: (_) => CustomerBillsPage(phone: widget.customerId),
                  ),
                );
                ...
                Navigator.push(
                  ...
                  CupertinoPageRoute(
                    builder: (_) =>
                        CustomerCreditsPage(customerId: widget.customerId),
                  ),
                );
              else
                Navigator.push(
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: () async {
                  if (_amt <= 0) return;
                  final cCol = await FirestoreService().getStoreCollection(
                    'customers',
                  );
                  final crCol = await FirestoreService().getStoreCollection(
                    'credits',
                  );
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              flex: 2,
                              child: Text(
                                e.debit > 0 ? e.debit.toStringAsFixed(0) : "0",
                                textAlign: TextAlign.right,
                                style: const TextStyle(
                                  color: kGoogleGreen,
                                  fontSize: 11,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text(
                        ...
                        style: TextStyle(color: kGrey400, fontSize: 12),
                      ),
                    ],
                  ),
                );
                  ...
                  Expanded(
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final cashReceived = (data['cashReceived'] ?? 0.0)
                            .toDouble();
                        final onlineReceived = (data['onlineReceived'] ?? 0.0)
                            .toDouble();
                        ...
                        Color color = isCancelled
                            ...
                            : (paymentMode == 'Cash'
                                  ...
                                  : (creditAmount > 0
                                        ? kOrange
                                        : Colors.purple));
                        HeroIcons icon = paymentMode == 'Cash'
                            ? HeroIcons.banknotes
                            : paymentMode == 'Online'
                            ? HeroIcons.qrCode
                            : paymentMode == 'Credit'
                        ...
                        String subtitle =
                            "${DateFormat('dd MMM yyyy').format(date)} • $paymentMode";
                        if (isCancelled) {
                          subtitle += " • CANCELLED";
            ...
            builder: (context, snapshot) {
              ...
              return ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  final customerName = (data['customerName'] ?? customerId)
                      .toString();
                  ...
                  return Container(
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(
                        color: isCancelled ? Colors.transparent : kGrey200,
                      ),
                    ),
                    child: Padding(
                      padding: const EdgeInsets.symmetric(
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  Builder(
                                    builder: (ctx) {
                                      final cleanPhone = customerId.replaceAll(
                                        RegExp(r'[\s\-+()]'),
                                        '',
                                      );
