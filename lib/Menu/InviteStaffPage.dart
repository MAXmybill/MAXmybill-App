...
import 'package:maxmybill/utils/translation_helper.dart'; // Ensure translation helper is available
...
class _InviteStaffPageState extends State<InviteStaffPage> {
  ...
  late Future<bool> _canInviteMoreFuture;
  ...
  void initState() {
    super.initState();
    _loadAvailableRoles();
    _canInviteMoreFuture = _canInviteMoreStaffForUi();
  }
  ...
  Future<bool> _canInviteMoreStaffForUi() async {
    try {
      ...
      // Keep UI permissive on transient fetch issues; submit flow re-checks limit.
      return true;
    }
  }
      ...
      if (currentStaffCount >= maxStaff) {
        if (mounted) {
          PlanPermissionHelper.showUpgradeDialog(
            context,
            maxStaff == 0 ? 'Staff Management' : 'Add Staff',
            uid: widget.uid,
