...
import 'package:maxmybill/Colors.dart' hide kWhite;
...
class _ReportsPageState extends State<ReportsPage> {
  ...
  Widget _buildMainReportsPage(
    ...
    bool hasFeaturePermission(String permission) {
      ...
      // Daybook is free but still permission-gated for staff.
      if (permission.toLowerCase() == 'daybook') {
        if (isAdmin) return true;
        return _permissions[_getPermissionKey(permission)] == true;
      }

      // Premium reports: keep visible if user has permission; lock UI/tap handles upgrade.
      if (isAdmin) return true;
...
class ReportPdfGenerator {
  static Future<void> generateAndDownloadPdf({
    ...
    try {
      ...
      final showWatermark = planProvider.isInitialized
          ? !planProvider.canRemoveWatermark()
          : !(await planProvider.canRemoveWatermarkAsync());
...
class _AnalyticsPageState extends State<AnalyticsPage> {
  ...
  double _periodRevenue = 0, _periodExpenseKpi = 0, _periodTax = 0;
  double _totalOnline = 0, _totalCash = 0;
  double _periodIncome = 0, _periodExpense = 0;
  double _totalRefunds = 0;
  int _periodSaleCount = 0;

  void _downloadPdf(BuildContext context) {
    final rows = [
      ['Revenue', '$_currencySymbol${_periodRevenue.toStringAsFixed(2)}'],
                ...
                builder: (context, expenseSnap) {
                  return StreamBuilder<QuerySnapshot>(
                    ...
                    builder: (context, stockSnap) {
                      ...
                      _periodExpenseKpi = periodExpenseKpi;
                      _periodTax = periodTax;
                      ...
                      _periodSaleCount = periodSaleCount;
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            height: 120,
            child: hasData
                ? BarChart(_getProfessionalBarData(activeHours))
...
class _SalesSummaryPageState extends State<SalesSummaryPage> {
  ...
  Widget _buildRevenueTimelineCard(Map<int, double> data) {
    ...
    final bool hasData = data.values.any((v) => v > 0);
      ...
      child: Column(
        ...
        children: [
          ...
          if (!hasData) ...[
            const SizedBox(
              height: 140,
              child: EmptyStateWidget(message: "No data found"),
            ),
          ] else ...[
            // Donut chart + legend
            Row(
...
class _FullSalesHistoryPageState extends State<FullSalesHistoryPage> {
  ...
  Map<String, dynamic> _calculateHeaderStats(List<DocumentSnapshot> docs) {
    double total = 0;
    int active = 0;
    int cancelled = 0;
    int returned = 0;
    ...
    for (var doc in docs) {
      ...
      final isReturned = status == 'returned' || d['hasBeenReturned'] == true;
      ...
      active++;
      total += double.tryParse(d['total']?.toString() ?? '0') ?? 0;
    }
      ...
      builder: (context, streamSnapshot) {
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            final headerStats = _calculateHeaderStats(allDocs);
            ...
            return Scaffold(
              ...
              body: Column(
                children: [
                  _buildHeader(
                    headerStats['total'] as double,
                    headerStats['active'] as int,
                    headerStats['cancelled'] as int,
                    headerStats['returned'] as int,
                  ),
                  DateFilterWidget(
                    selectedOption: _selectedFilter,
                    startDate: _startDate,
  ...
  Widget _buildTrendAreaChart(List<MapEntry<String, double>> trend) {
    ...
    final bool hasData = trend.any((e) => e.value > 0);
...
class _LowStockPageState extends State<LowStockPage> {
      ...
      builder: (context, streamSnapshot) {
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            expiredItems = _sortItems(expiredItems);
  ...
  Widget _buildExecutiveStockHeader(int low, int out, int expired) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
      decoration: BoxDecoration(
              ...
              if (expired > 0) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: kChartPurple.withOpacity(0.1)),
                  ),
                  child: Column(
                    children: [
                      Text(
...
class _TopCategoriesPageState extends State<TopCategoriesPage> {
  ...
  Map<String, Map<String, dynamic>> _cachedCategoryData = {};
  double _cachedTotalRevenue = 0;
  ...
  String _buildCategoryCacheKey(int docsCount) {
    return '${_startDate.millisecondsSinceEpoch}-${_endDate.millisecondsSinceEpoch}-$docsCount';
  }

  void _rebuildCategoryCache(List<DocumentSnapshot> docs) {
      ...
      builder: (context, streamSnapshot) {
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            final cacheKey = _buildCategoryCacheKey(docs.length);
            if (_categoryCacheKey != cacheKey) {
              _rebuildCategoryCache(docs);
              _categoryCacheKey = cacheKey;
            }
